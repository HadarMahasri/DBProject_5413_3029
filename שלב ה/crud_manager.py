...
class CRUDManager:
    ...
    def create_buttons_section(self, parent):
        """יצירת סקציית הכפתורים - עם הסתרת כפתור מחיקה לטבלאות מוגנות"""
        buttons_frame = tk.Frame(parent, bg='#1a4d3a', height=70)
        buttons_frame.pack(fill='x', padx=10, pady=5)
        buttons_frame.pack_propagate(False)
        
        # רשימת טבלאות מוגנות
        protected_tables = ['equipment', 'medical_event']
        ...
        # הוספת כפתור מחיקה רק לטבלאות לא מוגנות
        if self.current_table not in protected_tables:
            buttons_data.insert(2, ("🗑️ מחק", self.delete_record, '#8b4513'))
        else:
            # כפתור מוגבל עבור טבלאות מוגנות
            buttons_data.insert(2, ("🚫 מחיקה מוגבלת", self.show_delete_restriction, '#666666'))
    ...
    def show_delete_restriction(self):
        """הצגת הודעה על הגבלת מחיקה"""
        protected_info = {
            'equipment': {
                'reason': 'ציוד צבאי משמש בדרישות ובשימושים',
                'tables': 'requires, uses'
            },
            'medical_event': {
                'reason': 'אירועים רפואיים משמשים בטיפולים ובציוד',
                'tables': 'treatment, uses'
            }
        }
        
        if self.current_table in protected_info:
            info = protected_info[self.current_table]
            messagebox.showinfo(
                "🛡️ טבלה מוגנת", 
                f"🚫 מחיקה מטבלה זו מוגבלת\n\n"
                f"🔍 סיבה: {info['reason']}\n"
                f"📋 טבלאות תלויות: {info['tables']}\n\n"
                f"💡 לביטחון הנתונים, מחיקה אפשרית רק\n"
                f"   מהטבלאות התלויות (requires, uses)\n\n"
                f"🛡️ זאת למניעת פגיעה ביושרת המסד"
            )
    ...
    def delete_record(self):
        """מחיקת רשומה - עם בלימה לטבלאות עם תלויות"""
        config = self.table_configs[self.current_table]
        primary_key = config['primary_key']
        
        # בדיקה אם הטבלה מותרת למחיקה
        protected_tables = {
            'equipment': {
                'reason': 'טבלת הציוד משמשת בטבלאות requires ו-uses',
                'suggestion': 'מחק קודם את הדרישות והשימושים בציוד זה'
            },
            'medical_event': {
                'reason': 'אירועים רפואיים משמשים בטבלת treatment ו-uses',
                'suggestion': 'מחק קודם את הטיפולים הקשורים לאירוע זה'
            }
        }
        
        if self.current_table in protected_tables:
            info = protected_tables[self.current_table]
            messagebox.showwarning(
                "🚫 מחיקה לא מותרת", 
                f"❌ לא ניתן למחוק רשומות מטבלה זו\n\n"
                f"🔍 סיבה: {info['reason']}\n\n"
                f"💡 המלצה: {info['suggestion']}\n\n"
                f"🛡️ זאת למניעת פגיעה ביושרת הנתונים"
            )
            return
        
        # קבלת ערכי המפתח לטבלאות המותרות למחיקה
        if isinstance(primary_key, tuple):
            pk_values = []
        ...
        try:
            if isinstance(primary_key, tuple):
                ...
                # מפתח יחיד
                rows_affected, error = self.db.delete_record_simple(
